...
#        return return_string
        ...
        """if "subcategory" in values:
            ...
            for extra_field in request.env['mantenimiento.tipo.etiquetas'].sudo().search([('wsts_id','=', int(sub_category) )]):
                if "efield_" + str(extra_field.id) in values:
                    request.env['mantenimiento.tipo.etiquetas'].sudo().create({'wst_id': new_ticket_id.id, 'name': extra_field.name, 'value': values["efield_" + str(extra_field.id)] })
                else:
    ...
    def support_ticket_view_list(self, **kw):
        ...
        support_tickets = http.request.env['website.support.ticket'].sudo().search(['|', ('partner_id','=',http.request.env.user.partner_id.id), ('user_id','=',http.request.env.user.user_id.id),('person_name','=',http.request.env.user.name)])
